...
export const useSingleStepForm = (
  ...
  const {
    ...
    enabled: true, // Re-enabled after properly fixing infinite loop
  ...
  useEffect(() => {
    ...
    // Commented out to prevent infinite loop - loadProgress is already called in useFormProgress hook
    // if (formId && !progress) {
    //   loadProgress();
    // }
  }, [formId, fields, progress, isLoadingProgress]);
